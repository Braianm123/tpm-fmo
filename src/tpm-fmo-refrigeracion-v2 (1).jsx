...
export default function TPMFMO() {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (window.storage) {
          const r = await window.storage.get("tpm-fmo-v4-demo");
          if (r && r.value) {
            const d = JSON.parse(r.value);
            setEquipos(d.equipos || []); setAtenciones(d.atenciones || []); setLecturas(d.lecturas || []); setJornadas(d.jornadas || []);
          }
        }
      } catch (e) {}
      setCargado(true);
  ...
  useEffect(() => {
    if (!cargado) return;
    (async () => { try { if (window.storage) await window.storage.set("tpm-fmo-v4-demo", JSON.stringify({ equipos, atenciones, lecturas, jornadas })); } catch (e) {} })();
  }, [equipos, atenciones, lecturas, jornadas, cargado]);
  ...
  const agregarEquipo = (e) => { setEquipos((xs) => [...xs, { id: uid(), ...e }]); notificar("Equipo agregado al inventario"); };
  ...
  const registrarAtencion = (a) => {
    setAtenciones((as) => [{ id: uid(), ...a }, ...as]);
    if (a.tipo === "preventiva") {
      setEquipos((xs) => xs.map((e) => (e.id === a.equipoId ? { ...e, ultimoPrev: a.fecha } : e)));
  ...
  const registrarJornada = (j) => {
    ...
    setEquipos((xs) => xs.map((e) => (gerenciaDe(e) === j.gerencia ? { ...e, ultimoPrev: fecha } : e)));
    notificar(`Jornada registrada · ${j.gerencia} · semáforo del área reiniciado`);
  };
  const registrarLectura = (l) => {
    setLecturas((ls) => [{ id: uid(), ...l }, ...ls]);
  ...
  const cargarReal = () => {
    const ejecutar = () => {
      setEquipos(cargarInventarioFMO());
      setAtenciones([]); setLecturas([]); setTab("tablero");
      notificar("Programa 2026 cargado · " + INVENTARIO_FMO.length + " equipos");
    };
  ...
  const vaciarTodo = () => {
    pedirConfirmacion("¿Borrar TODOS los equipos, atenciones y lecturas de este dispositivo? Esta acción no se puede deshacer.", () => {
